...
我们把组成 buffer 的非门（如图 d）替换成或非门或者与非门，就可以方便地修改存储的信息内容，这就是 $SR$ Latch。稍加思考不难发现，这样的一个 $SR$ Latch 其实是异步的，因而输入 S 和 R 的先后抵达可能导致电路偏离预期而进入错误的状态。如果要实现同步，或者说通过使能控制输入 S 和 R 产生作用的时间，我们可以设计出 Clocked $SR$ Latch。但是，这个 Latch 仍然存在一种未定义状态，即 C、S 和 R 均置`1`。

对于 Clocked $SR$ Latch 而言，用 S 和 R 均置`0`来表示保持态是多余的（因为 C 置`0`即可表示保持态），而且上述的未定义状态也很麻烦。所以，我们把 S 和 R 强制设定为相反的两个输入，用 $D$ 和 $\overline{D}$ 来代替 $S$ 和 $R$，这就是 $D$ Latch。
